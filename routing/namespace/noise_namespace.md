...
- repo: string  
- label_name: string
- milestone_number: integer (optional)

### Addressability (Sender and Receiver)
#### View all labels defined in the repository
- **GUI URL:** https://github.com/{owner}/{repo}/labels
#### View a specific label
- **GUI URL:** https://github.com/{owner}/{repo}/labels/{label_name}
- **Notes:** Shows metadata for a single label, including color and description.
#### View labels used by issues within a milestone
- **GUI URL:** https://github.com/{owner}/{repo}/milestone/{milestone_number}
- **Notes:** Labels appear as aggregated metadata across milestone issues.

## Notes and Boundaries
- Creating, updating, or deleting labels constitute **snapshot-mutating actions** and are therefore out of scope.
- The model does **not** treat individual label names as distinct interaction classes.
- Query filtering and sorting are considered **presentation-layer variations** and are not modeled separately.
