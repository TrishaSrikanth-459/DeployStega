...
class NLSHuffmanEncoder:
    ...
    def _encode_chunk(self, bits: List[int], max_tokens: int = 25) -> Tuple[str, int]:
        """
        Encode bits into text using Huffman-over-GPT2 with KV-cache.
        KV-cache makes each step O(1) instead of O(T) — ~5-10x faster on CPU.
        Returns (text, bits_consumed).
        """
        bos = self.tokenizer.bos_token_id or self.tokenizer.eos_token_id or 0
        prev_id = torch.tensor([[bos]], dtype=torch.long, device=self.device)
        ...
        for _ in range(max_tokens):
            if i >= len(bits):
                break
            outputs = self.model(prev_id, past_key_values=past_key_values, use_cache=True)
            ...
            prev_id = torch.tensor([[selected_id]], dtype=torch.long, device=self.device)
    ...
    def encode_to_chunks(self, num_chunks: int, seed: int = 42) -> List[str]:
        ...
        Each chunk gets a fresh KV-cache; no cross-chunk state.
        """
        ...
        for _ in range(num_chunks):
            ...
            text, _ = self._encode_chunk(bits, max_tokens=25)
