...
SOURCE_LEAK_LINE_PATTERNS = [
    ...
    for pattern in (
        ...
        r"^\s*ghstack dependencies\s*:",
        r"^\s*Stack from\s*:",
        r"^\s*Stack from\b.*:\s*",
        r"^\s*(?:Codecov\)?\s*)?Report Attention:\s*Patch coverage\b.*$",
        r"^\s*Codecov\)?\s*Report\b.*Patch coverage\b.*$",
        ...
        r"^\s*The solution for this issue has been .*deployed to production.*$",
        r"^\s*Here is the list of pull requests that resolve this issue.*$",
        r"^\s*For reference, here are some details about the assignees.*$",
        r"^\s*.*requires payment.*$",
        r"^\s*.*payment will be issued.*$",
        r"^\s*.*subject to a .*regression period.*$",
    )
]
...
SOURCE_LABEL_PREFIX_RE = re.compile(
    r"^\s*(?:\[[A-Za-z0-9_.:/#,+ -]{1,40}\]\s*){1,5}[:\-\s]*"
)
SOURCE_TRAILING_PR_RE = re.compile(r"\s*\(?#(?:PR)?\d{3,}\)?\s*$", re.IGNORECASE)
...
SOURCE_HEADING_LABEL_RE = re.compile(
    r"^\s*(summary|test plan|tests?|fixes?|closes?|resolves?|motivation|context|changes?|issue|problem|solution|release notes?|pr|generic|pr url|pull request url|issue url|url)\s*:?\s*",
    re.IGNORECASE,
)
SOURCE_URL_LABEL_RE = re.compile(r"\b(?:pr|pull request|issue)?\s*url\b", re.IGNORECASE)
SOURCE_INLINE_TEMPLATE_TAIL_RE = re.compile(
    r"\b(?:"
    r"Pre-launch Checklist|Pre-merge author checklist|Pre-merge reviewer checklist|"
    r"Manual testing steps|Screenshots/Recordings|Identify risks|"
    r"If you need help, consider asking|Please refer to the .*?documentation|"
    r"I read (?:the )?(?:\[)?Contributor Guide(?:\])?|"
    r"I read (?:and followed )?(?:the )?(?:\[)?Flutter Style Guide(?:\])?|"
    r"I signed (?:the )?(?:\[)?CLA(?:\])?|"
    r"I listed at least one issue|All existing and new tests are passing|"
    r"Contribution Guidelines|Submitting Patches|When filling out the below checklist|"
    r"Open in GitHub Codespaces|"
    r"(?:\(\s*\)\s*)?Backport\s+This will backport the following commits|"
    r"This will backport the following commits|"
    r"Please refer to the Backport tool documentation|"
    r"cherry picked from commit|"
    r"The solution for this issue has been .*?deployed to production|"
    r"Copybara import of the project|"
    r"Merging this change closes|"
    r"COPYBARAINTEGRATEREVIEW|"
    r"PiperOrigin-RevId|"
    r"fbshipit-source-id|"
    r"GHA Pulled By|"
    r"Pull Request resolved|"
    r"Approved by|"
    r"Differential Revision|"
    r"ghstack-source-id|"
    r"Here is the list of pull requests that resolve this issue|"
    r"For reference, here are some details about the assignees|"
    r"[^.]{0,80}requires payment|[^.]{0,80}payment will be issued"
    r")\b.*$",
    re.IGNORECASE,
)
...
def _strip_inline_source_markers(line: str) -> str:
    """Remove import/scaffold markers that are not semantic developer prose.

    These transformations are intentionally symmetric and conservative: they
    delete GitHub-import metadata such as stacked bracket tags, PR numbers, and
    bot labels while leaving the surrounding natural-language explanation.
    """
...
def normalize_semantic_text_for_detection(text: Any) -> str:
    ...
    # Markdown links keep their label; image badges/buttons are source UI
    # artifacts rather than developer prose. Handle broken imported image syntax
    # too (common after JSON/CSV scraping).
    text = re.sub(r"!\[[^\]]*\]\([^)]*\)", " ", text)
    text = re.sub(r"!\[[^\]]*\]\([^\s)]*", " ", text)
    text = re.sub(r"\bReviewed by [^.]{2,140}\.\s*", " ", text, flags=re.IGNORECASE)
    # GitHub mirroring/import labels are source transport artifacts when they
    # survive inline in scraped PR/Issue bodies. Remove the marker itself while
    # retaining the surrounding substantive developer text.
    text = re.sub(r"\bImported from GitHub PR\b[:\s-]*", " ", text, flags=re.IGNORECASE)
    text = re.sub(r"\bImported from GitHub issue\b[:\s-]*", " ", text, flags=re.IGNORECASE)
    text = re.sub(r"\bCOPYBARAINTEGRATEREVIEW\s*=.*?(?=$|\n)", " ", text, flags=re.IGNORECASE)
    text = re.sub(r"\bPiperOrigin-RevId\s*:\s*\S+", " ", text, flags=re.IGNORECASE)
    text = re.sub(r"\bfbshipit-source-id\s*:\s*\S+", " ", text, flags=re.IGNORECASE)
    text = re.sub(r"\bGHA Pulled By\s*:\s*.*?(?=$|\n)", " ", text, flags=re.IGNORECASE)
    text = re.sub(r"\s*--\s*by\s+[^:]{2,100}\s*:\s*", " ", text, flags=re.IGNORECASE)
    # Malformed scraped markdown links are source-format artifacts. Keep any
    # visible label text, but remove dangling link syntax so the classifier
    # cannot learn CSV/GitHub renderer breakage.
    text = re.sub(r"\[\[?([^\[\]]{2,180})\]\([^)]*", r"\1", text)
    text = re.sub(r"\]\(", " ", text)
    ...
    for raw_line in text.splitlines():
        ...
        line = re.sub(r"^\s*#+\s*", "", line)
        line = line.replace("`", "")
        # Remove inline PR-template tails that survive as one-line imported
        # bodies (e.g. Flutter/Backport checklists). These are source/template
        # scaffolds rather than developer semantics, and are stripped
        # symmetrically before both smoke diagnostics and semantic features.
        line = SOURCE_INLINE_TEMPLATE_TAIL_RE.sub(" ", line)
        # Emoji aliases and reaction/status decorations are frequent import/bot
        # artifacts in benign GitHub bodies. Remove the markers only, not the
        # surrounding developer prose.
        line = re.sub(r":(?:[a-z0-9_+-]{2,40}):", " ", line, flags=re.IGNORECASE)
        line = re.sub(r"\bLink\s*:\s*$", " ", line, flags=re.IGNORECASE)
        line = re.sub(r"</?[^>]+>", " ", line)
        line = re.sub(r"&(?:amp|lt|gt|quot|apos);", " ", line)
        line = line.replace("|", " ")
        line = re.sub(r"[*_]{1,3}([^*_]+)[*_]{1,3}", r"\1", line)
        line = re.sub(r"[-=]{5,}", " ", line)
        line = _strip_inline_source_markers(line)
    ...
    # Re-run inline source-marker cleanup once after line joining. Some scraped
    # PR bodies arrive as a single collapsed line, and the first cleanup pass can
    # expose metadata tails (Copybara, GHA, fbshipit) that were previously glued
    # to surrounding text. This remains symmetric for benign/covert inputs.
    normalized = _strip_inline_source_markers(normalized)
    normalized = SOURCE_INLINE_TEMPLATE_TAIL_RE.sub(" ", normalized)
    normalized = re.sub(r"\bCOPYBARAINTEGRATEREVIEW\s*=.*$", " ", normalized, flags=re.IGNORECASE)
    normalized = re.sub(r"\bGHA Pulled By\s*:\s*.*$", " ", normalized, flags=re.IGNORECASE)
    normalized = re.sub(r"\bfbshipit-source-id\s*:\s*\S+", " ", normalized, flags=re.IGNORECASE)
    normalized = re.sub(r"\bPiperOrigin-RevId\s*:\s*\S+", " ", normalized, flags=re.IGNORECASE)
    normalized = re.sub(r"\s+", " ", normalized).strip()
    # Whole-event bot/status reports are authoring-system artifacts, not
    # comparable human semantic content. Drop after line-level cleanup too,
    # because scraped Codecov bodies often arrive as one collapsed line.
    if re.match(r"^Codecov\)?\s*Report\b", normalized, flags=re.IGNORECASE):
        return ""
    if re.match(r"^Stack from\b.*:\s*", normalized, flags=re.IGNORECASE):
        return ""
    return normalized
