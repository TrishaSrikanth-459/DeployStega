...
NO SMOOTHING - Raw empirical distributions only.
"""
import json
import pickle
...
def extract_features(dataset: BenignDataset) -> Dict:
    """Extract behavioral features - NO SMOOTHING, RAW DISTRIBUTIONS ONLY."""
    print("=" * 70)
    print("EXTRACTING FEATURES (RAW EMPIRICAL DISTRIBUTIONS)")
    print("=" * 70)
    print()
    ...
    # Extract raw distributions (NO STATISTICS, NO SMOOTHING)
    ...
    print("=" * 70)
    print("DISTRIBUTION SIZES (RAW DATA)")
    print("=" * 70)
    print(f"  ft_intra_user_timing: {len(timing):,} values")
    print(f"  fsession_length: {len(session):,} values")
    print(f"  faccess_transition_matrix: {len(transition)} classes")
    print(f"  f_event_type_frequency: {len(frequency)} event types")
    print(f"  f_artifact_revisit: {len(revisit[0]):,} users")
    print("=" * 70)
    print()

    return {
        "metadata": {
            "description": "Empirical behavioral features from GitHub Archive - RAW DISTRIBUTIONS (NO SMOOTHING)",
            ...
            "note": "Distributions frozen exactly as observed - no fitting, no smoothing"
        },
        
        # RAW timing deltas (ALL values, not just 1000 samples)
        "ft_intra_user_timing": list(timing),
        
        # RAW session lengths (ALL values)
        "fsession_length": list(session),
        
        # RAW transition matrix (exact empirical counts)
        "faccess_transition_matrix": transition,
        
        # RAW event type frequencies (exact empirical probabilities)
        "f_event_type_frequency": frequency,
        
        # RAW artifact revisit data (ALL values)
        "f_artifact_revisit": {
            "revisit_rate": list(revisit[0]),
            "unique_artifacts": list(revisit[1]),
            "max_revisits": list(revisit[2])
        }
    }
...
def main(resume: bool = True):
    ...
    print("NO SMOOTHING - RAW EMPIRICAL DISTRIBUTIONS ONLY")
    print("=" * 70)
    print()
    ...
    try:
        ...
        print(f"  Timing deltas: {len(behavioral_priors['ft_intra_user_timing']):,}")
        print(f"  Session lengths: {len(behavioral_priors['fsession_length']):,}")
        print()
        print("RAW DISTRIBUTIONS SAVED:")
        print("  ✓ NO statistics computed")
        print("  ✓ NO smoothing applied")
        print("  ✓ NO fitting performed")
        print("  ✓ Exact empirical observations preserved")
        print("=" * 70)
        print()
